...
export class NnmHygieneReceipts {
  ...
  async latest(input) {
    ...
    for (const line of content.split(/\r?\n/u)) {
      ...
      if (parsed.project_fingerprint !== project || Date.parse(parsed.completed_at) < input.since) continue;
...
function normalize(value) {
  ...
  for (const key of ['receipt_id', 'session_id', 'project_fingerprint']) bounded(value[key]);
