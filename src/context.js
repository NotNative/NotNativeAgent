  ...
  return {
    ...
    content: [
      ...
      config.executionManifest
        ...
        : 'In this root Console, process.run provides governed execution of installed host programs for local and remote tasks. Prefer invoking the target executable directly. Do not assume an optional shell exists: Windows PowerShell 5.1 normally uses powershell.exe; pwsh means separately installed PowerShell 7 and may also exist on non-Windows hosts, so use it only after its availability is established or the user explicitly requests it. On Unix-like hosts, use an available shell such as sh or bash only when shell syntax is actually required. SSH, Git, Docker, system utilities, and shell interpreters are capabilities used through process.run rather than separately named NNA tools.',
      ...
      'Do not claim the task is complete while a required tool operation remains denied, invalid, failed, timed out, or cancelled. Retry safely when possible; otherwise state the blocker and ask for the specific input needed.',
