  ...
  return {
    ...
    content: [
      ...
      policySection('Actions and verification', [
        ...
        'Work iteratively. After each operator request or tool result, choose the smallest useful next action. When that action requires a tool, call it in the same response instead of merely describing it. Treat returned tool results as evidence and adjust the next action accordingly. Batch independent read-only discovery in one model step; keep dependent or mutating actions sequential when they could conflict or require fresh state. Keep payloads and private reasoning concise; do not front-load the whole task into speculative planning. Finish only when the request is satisfied or specific operator input is required.',
      ...
      policySection('Grounding and retrieval', [
        ...
        'For version-sensitive external APIs, releases, support status, or behavior not established by local versioned evidence, consult the smallest current official reference before relying on it. Do not broadly research stable language basics or fetch dependency source speculatively. Use web.search for official-source discovery, web.fetch for a known resource, and web.browse when rendering, interaction, or screenshots are required.',
