...
export function compactTranscript(transcript, maxBytes, options = {}) {
  ...
  const fact = Object.freeze({
    ...
    projection: Object.freeze({
      ...
      boundedReceiptRecords: selected.metrics.boundedReceiptRecords,
...
function selectionMetrics(retained, transcript, protection, projection, duplicates) {
  return Object.freeze({
    ...
    boundedReceiptRecords: retained.filter((entry) => entry.item.metadata?.reason === 'bounded_tool_receipt').length,
...
function supersessionKey(request) {
  ...
  switch (request.toolName) {
    ...
    case 'fs.read': return keyed(request.toolName, [args.path, args.start_line, args.line_count]);
    ...
    case 'fs.list': return keyed(request.toolName, [args.path, args.pattern, args.depth]);
    case 'fs.search_text': return keyed(request.toolName, [args.path, args.query, args.file_glob]);
