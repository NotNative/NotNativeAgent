...
export async function discoverLocalProvider(options = {}) {
  ...
  const results = await Promise.all(LOCAL_ENDPOINTS.map((endpoint) => probe(endpoint).catch(() => null)));
...
async function loadManifestFile(path) {
  return resolveManifest(await loadManifestDocument(path));
}
...
function providerManifest(endpoint, model, id) {
  ...
  const trustZone = ['localhost', '127.0.0.1', '::1'].includes(host) ? 'loopback'
    : /^(10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.)/u.test(host) ? 'private_network' : 'public_network';
  return {
    persistence: 'durable',
...
async function probeEndpoint(endpoint) {
  const response = await fetch(`${endpoint}/models`, { signal: AbortSignal.timeout(800) });
