...
export function toolContinuationHint(items, fallback = null) {
  ...
  if (invalid.length > 0) {
    if (invalid.some((item) => item.result?.tool_name === 'work_plan')) {
      return 'The durable plan update was invalid. Treat plan synchronization as bookkeeping, not as completion of or a blocker to independent substantive work. Correct the exact field reported by the tool. Never invent a task id: copy an id only from the current work_status result, and omit id for every new task so NNA can assign it. Use work_task_update when only one existing task status changed; use work_plan only for a complete snapshot change. If another independent task action remains available, continue it before retrying plan synchronization. Do not repeat unchanged arguments.';
    }
