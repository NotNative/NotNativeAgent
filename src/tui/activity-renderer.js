...
export function decorateToolActivityLine(line, lineKind, paint) {
  if (lineKind?.startsWith('tool_status:')) {
    ...
    if (status === 'diagnostic') return paint(TUI_THEME.warning, line);
...
export function compactActivityRows(records) {
  return toolCalls(records).map((item) => {
    ...
    return `    ${toolSymbol(item.result?.status, item.result?.observation_outcome, toolHasDiagnostics(item.result))} ${item.tool}${target}${timing}${toolObservationSuffix(item.result)}${toolDiagnosticSuffix(item.result)}${failure}`;
  });
}
...
export function activityDetailRows(records, width, wrap) {
  ...
  for (const item of toolCalls(records)) {
    ...
    lines.push(...wrap(`    ${toolSymbol(item.result?.status, item.result?.observation_outcome, toolHasDiagnostics(item.result))} ${item.tool}${item.target ? ` (${item.target})` : ''}${boundary ? ` | ${boundary}` : ''}`, width));
...
function resultDetail(record) {
  ...
  if (record.diagnostic_outcome === 'stderr_present') values.push('stderr present');
  if (record.diagnostic_visibility === 'reduced_by_script') values.push('diagnostic visibility reduced');
  const failure = toolFailureText(record); if (failure) values.push(failure);
  return values.join(' | ');
}

export function toolSymbol(status, observationOutcome = null, diagnostics = false) {
  if (!status) return '-';
...
function toolDiagnosticSuffix(record) {
  if (!toolHasDiagnostics(record)) return '';
  const values = [record.diagnostic_outcome === 'stderr_present' ? 'stderr present' : null,
    record.diagnostic_visibility === 'reduced_by_script' ? 'visibility reduced' : null].filter(Boolean);
  return ` | succeeded · ${values.join(' · ')}`;
}
