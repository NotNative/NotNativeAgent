...
const conformance = await json('docs/conformance/windows-linux-2026-08-01.json');
const versionLedger = await json('RELEASE_VERSIONS.json');
const productSource = await readFile(join(root, 'src/product.js'), 'utf8');
if (packageJson.license !== 'Apache-2.0') errors.push('package license is not Apache-2.0');
if (Object.keys(packageJson.dependencies ?? {}).length > 0) errors.push('runtime dependencies are not empty');
if (sbom.spdxVersion !== 'SPDX-2.3') errors.push('SBOM format is invalid');
verifyDependencies();
...
if (packageJson.nna_version !== releaseVersion || packageJson.version !== mappedPackageVersion) errors.push('package versions disagree with VERSION');
if (sbom.packages?.[0]?.versionInfo !== releaseVersion) errors.push('SBOM version disagrees with VERSION');
if (!productSource.includes(`VERSION = '${releaseVersion}'`)) errors.push('runtime version disagrees with VERSION');
if (conformance.release !== releaseVersion) errors.push('platform conformance version disagrees with VERSION');
...
for (const platform of ['windows', 'linux', 'macos']) {
  if (!conformance.results?.some((item) => item.environment?.toLowerCase().includes(platform))) {
...
for (const required of [
  ...
  'docs/conformance/windows-linux-2026-08-01.json',
...
function verifyDependencies() {
  ...
  for (const item of packages.filter((candidate) => candidate.name !== packageJson.name)) {
    if (typeof item.versionInfo !== 'string' || item.versionInfo.length === 0) errors.push(`SBOM dependency version is missing: ${item.name}`);
...
async function verifyReleaseHashes() {
  ...
  for (const name of actualPaths) {
    ...
    const actualHash = createHash('sha256').update(await readFile(join(root, name))).digest('hex');
...
async function candidateDigest() {
  ...
  for (const file of files) {
    digest.update(file.name); digest.update('\0'); digest.update(await readFile(file.path)); digest.update('\0');
  }
...
function releaseEligible(path) {
  ...
  return !['docs/planning/', 'node_modules/', '.git/', '.nna/', '.tmp/', '.tmp-npm-cache/', '.npm-cache/']
...
async function json(path) {
  ...
  catch { errors.push(`${path} is invalid`); return {}; }
}
