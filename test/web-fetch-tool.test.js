...
test('web.fetch follows bounded redirects and returns attributed UTF-8 text', async () => {
  ...
  assert.match(definition.purpose, /Fetch bounded HTTP\(S\) text/iu);
  assert.match(definition.purpose, /public URL|trusted private origin/iu);
  assert.doesNotMatch(definition.purpose, /authoritative source|must|always|first use/iu);
