...
test('active pressure tiers use conservative local-model boundaries', () => {
  assert.deepEqual(CONTEXT_PRESSURE, {
    receipts: 0.45, checkpoint: 0.60, aggressive: 0.70, compact: 0.75,
  });
  assert.equal(pressureTier(44_999, 100_000), 'none');
  assert.equal(pressureTier(45_000, 100_000), 'receipts');
  assert.equal(pressureTier(60_000, 100_000), 'checkpoint');
