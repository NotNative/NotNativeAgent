...
test('registry exposes workspace operations and packaged self-guidance', async () => {
  ...
  assert.deepEqual(registry.snapshot().map((item) => item.name).sort(), [
    ...
    'image_inspect', 'nna_diagnose_turn', 'nna_list_sessions', 'nna_read_guidance', 'nna_search_guidance', 'process_run', 'project_verify', 'ref_inspect', 'ref_store', 'shell_run', 'system_time', 'tool_search', 'web_browse', 'web_fetch', 'web_search',
  ]);
