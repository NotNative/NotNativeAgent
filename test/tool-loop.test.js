...
test('registry exposes workspace operations and packaged self-guidance', async () => {
  ...
  assert.deepEqual(registry.snapshot().map((item) => item.name).sort(), [
    ...
    'nna.diagnose_turn', 'nna.list_sessions', 'nna.read_guidance', 'nna.search_guidance', 'process.run', 'project.verify', 'shell.run', 'tool.search', 'web.browse', 'web.fetch', 'web.search',
  ]);
