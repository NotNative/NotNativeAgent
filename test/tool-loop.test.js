...
test('registry exposes workspace operations and packaged self-guidance', async () => {
  ...
  assert.deepEqual(registry.snapshot().map((item) => item.name).sort(), [
    ...
    'image.inspect', 'nna.diagnose_turn', 'nna.list_sessions', 'nna.read_guidance', 'nna.search_guidance', 'process_run', 'project_verify', 'ref.inspect', 'ref.store', 'shell_run', 'system.time', 'tool_search', 'web.browse', 'web.fetch', 'web.search',
  ]);
