...
test('AC-TURN-03 safe read is reviewed, executed, reinjected, and completed', async () => {
  ...
  assert.deepEqual(outputs.filter((item) => item.type === 'tool_status' && item.tool !== 'turn_finish').map((item) => item.status), [
...
test('AC-EVENT-06/AC-TOOL-05 duplicate tool identity reuses the terminal result without execution', async () => {
  ...
  assert.equal(engine.reviewerAudit().filter((item) => item.tool !== 'turn_finish').length, 1);
  assert.equal(engine.transcript.filter((item) => item.type === 'tool_result' && item.toolName !== 'turn_finish').length, 1);
  assert.equal(output.filter((item) => item.status === 'running' && item.tool !== 'turn_finish').length, 1);
...
test('AC-TOOL-01 unknown tool never reaches review or execution', async () => {
  ...
  assert.equal(result.outcome, 'completed');
  assert.equal(engine.reviewerAudit().filter((item) => item.tool !== 'turn_finish').length, 0);
...
test('registry exposes workspace operations and packaged self-guidance', async () => {
  ...
  assert.deepEqual(registry.snapshot().map((item) => item.name).sort(), [
    'code_diagnostics', 'fs.list_directory',
    'fs.metadata', 'fs.move_file', 'fs.read_lines', 'fs.read_text', 'fs.write_text', 'fs_copy_file', 'fs_create_directory', 'fs_delete_file', 'fs_directory', 'fs_edit_lines', 'fs_edit_text', 'fs_glob', 'fs_list', 'fs_read', 'fs_search_text', 'git_inspect',
...
test('AC-ARCH-02/AC-TURN-05 multiple tool calls retain nested lifecycle and deterministic request order', async () => {
  ...
  assert.equal(result.outcome, 'completed');
  assert.deepEqual(
    engine.transcript.filter((item) => item.type === 'tool_result' && item.toolName !== 'turn_finish').map((item) => item.providerCallId),
