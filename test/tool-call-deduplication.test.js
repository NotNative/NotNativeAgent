...
test('exact same-batch tool calls collapse before governance while distinct calls retain order', () => {
  const first = call('call-1', 'fs_read', { path: 'README.md', options: { end: 2, start: 1 } });
  const duplicate = call('call-2', 'fs_read', { options: { start: 1, end: 2 }, path: 'README.md' });
  const distinct = call('call-3', 'fs_read', { path: 'package.json' });
  ...
  assert.equal(result.suppressed[0].toolName, 'fs_read');
...
test('malformed calls remain visible to ordinary validation and recovery', () => {
  const invalidA = Object.freeze({ providerCallId: 'bad-1', name: 'fs_read', args: {}, invalid: { code: 'malformed' } });
  const invalidB = Object.freeze({ providerCallId: 'bad-2', name: 'fs_read', args: {}, invalid: { code: 'malformed' } });
