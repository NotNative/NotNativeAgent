...
import { contextPressureScale } from '../src/reliability/provider-recovery.js';
...
test('new verified progress settles stale no-progress budgets across categories', () => {
  const recovery = new RecoverySupervisor({ localLimit: 3, ladder: ['nudge', 'nudge'] });
  const unchangedFailure = 'one unresolved reviewed tool outcome';
  assert.equal(recovery.continuation('unresolved_reviewed_tool_outcome', unchangedFailure).progress, true);
  assert.equal(recovery.continuation('unresolved_reviewed_tool_outcome', unchangedFailure).count, 1);
  ...
  const resumedFailure = recovery.continuation('unresolved_reviewed_tool_outcome', unchangedFailure);
  assert.equal(resumedFailure.count, 1);
  assert.equal(resumedFailure.action.action, 'nudge');
});
