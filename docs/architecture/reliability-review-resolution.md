...
| Conversation work could not round-trip its own status shape | Resolved by `b8e1b95`. `work_plan` and `work.status` share one lossless provider contract with revision protection and round-trip tests. |
