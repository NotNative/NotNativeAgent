...
| NNA guidance/diagnostics | `nna.search_guidance`, `nna.read_guidance`, `nna_diagnose_turn`, `nna.list_sessions`, `nna.mcp_status`, `nna.mcp_test` | Pass. Selector conflicts and exact-id behavior are now visible through retained field descriptions. |
| Web and images | `web.search`, `web.fetch`, `web.browse`, `image_inspect` | Hardened. `web.browse` remains one session-oriented action tool to avoid eight competing browser schemas; its provider-visible action field gives the exact argument mapping for every action. `image_inspect@2` emits a normalized verdict whose visual authority can be superseded only by newer visual evidence. |
