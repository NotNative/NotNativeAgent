...
| NNA guidance/diagnostics | `nna_search_guidance`, `nna_read_guidance`, `nna_diagnose_turn`, `nna_list_sessions`, `nna.mcp_status`, `nna.mcp_test` | Pass. Selector conflicts and exact-id behavior are now visible through retained field descriptions. |
...
| Notifications/history | `notification_telegram`, `session.search_history`, `session.read_history` | Pass. Message, filters, indexes, and surrounding-record bounds match runtime checks. |
