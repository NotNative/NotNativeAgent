...
| Filesystem observation | `fs.list_directory`, `fs.read_text`, `fs.read_lines`, `fs_glob`, `fs_search_text`, `fs.metadata`, `fs_read`, `fs_list` | Pass. Defaults, path/pattern roles, and receipt behavior are provider-visible; numeric safety bounds remain runtime-enforced. |
