...
| Filesystem observation | `fs_list_directory`, `fs.read_text`, `fs_read_lines`, `fs_glob`, `fs_search_text`, `fs_metadata`, `fs_read`, `fs_list` | Pass. Defaults, path/pattern roles, and receipt behavior are provider-visible; numeric safety bounds remain runtime-enforced. |
