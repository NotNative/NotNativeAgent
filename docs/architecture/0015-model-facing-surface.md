...
Provider tool results use the versioned `nna.tool-result.v3` envelope. The canonical lifecycle,
review outcome, trust label, and content projection are distinct fields. There is no duplicate
`status` field. Projection metadata states the original,
...
whole-snapshot operation; `work_task_update` is the normal one-task transition and therefore
