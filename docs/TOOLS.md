...
The granular `fs_list_directory`, `fs_glob`, `fs_metadata`, `fs_read_text`, `fs_read_lines`,
`fs_create_directory`, `fs_edit_lines`, `fs_copy_file`, `fs_move_file`, and `fs_delete_file`
...
`python -c`, or similar argv. The agent stores the source as a bounded draft with `ref_store`
...
- `fs_edit_lines`: replace an inclusive numbered range previously displayed by `fs_read_lines`.
...
- `fs_metadata`: inspect bounded file or directory metadata without reading content.
...
- `fs_copy_file` and `fs_move_file`: copy or move an exact-hash regular file to a new
...
- `web_search`: query the ordered SearXNG profile chain and return bounded source
  summaries. Failure, degradation, or zero results advance to the next configured profile.
  The result distinguishes matches, valid empty searches, and upstream-engine degradation.
  It includes bounded profile attempts, engine failure details, and recovery guidance.
  Exhausted-profile guidance directs the agent to ask the user to add a backup through
  `/websearch`; it identifies `https://searx.space/` as a directory of public instances.
  NNA never selects or configures a public instance without the user.
  A profile endpoint may intentionally be loopback, private-network, or public;
