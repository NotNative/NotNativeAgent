...
`nna_search_guidance` and `nna_read_guidance` are foundational tools; the model grounds its
...
belongs to the current conversation snapshot. `nna_mcp_test` independently negotiates a
