...
Hosts select one of the operator's saved NNA provider profiles with
`nna host -provider PROFILE_ID` or the bounded `manifest.provider_profile_id` field on the
`initialize` command. Both paths resolve the same stable profile ID. When a profile is explicitly selected, NNA supplies
...
secret-free effective route under `provider` with `profile_id`, `display_name`, `endpoint`,
and `model`. Legacy flat fields remain readable during migration. Legacy hosts may also
continue supplying a complete provider route in the authenticated manifest.

NNO provider/secret management uses the separately authenticated ephemeral integration
...
unavailable unless `skill_search` and `skill_load` are explicitly granted. Skill content
